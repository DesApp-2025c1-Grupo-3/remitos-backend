...
// Schema específico para actualizar solo los campos de mercadería
const mercaderiaUpdateSchema = Joi.object().keys({
  ...
  valorDeclarado: Joi.number().required().min(0).messages({
  ...
  volumenMetrosCubico: Joi.number().required().min(0).messages({
  ...
  pesoMercaderia: Joi.number().required().min(0).messages({
...
module.exports = mercaderiaUpdateSchema;
