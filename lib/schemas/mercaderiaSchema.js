...
const mercaderiaSchema = Joi.object().keys({
  // Relación FK (requerida)
  tipoMercaderiaId: Joi.number().integer().min(1).required().messages({
    "any.required": "ID del tipo de mercadería es requerido",
    "number.base": "ID del tipo de mercadería debe ser un número",
    "number.integer": "ID del tipo de mercadería debe ser un número entero",
    "number.min": "ID del tipo de mercadería debe ser mayor a 0",
  }),

  // Campos de valor y dimensiones
  ...
  // Campos de cantidades (opcionales)
