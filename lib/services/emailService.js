...
class EmailService {
  ...
  initialize() {
    ...
    if (process.env.EMAIL_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      ...
      this.transporter = nodemailer.createTransport({
        ...
        // Configuraciones adicionales para Render con seguridad más permisiva
        tls: {
  ...
  async sendMailViaAPI(emailData) {
    try {
      ...
      console.log(`   EMAIL_FROM: ${senderEmail ? 'Configurado (' + senderEmail + ')' : 'No configurado'}`);
      console.log(`   EMAIL_PASS: ${apiKey ? 'Configurado (longitud: ' + apiKey.length + ')' : 'No configurado'}`);
      console.log(`   EMAIL_PASS primeros 10 chars: ${apiKey ? apiKey.substring(0, 10) + '...' : 'No disponible'}`);
      console.log(`   EMAIL_PASS últimos 10 chars: ${apiKey ? '...' + apiKey.substring(apiKey.length - 10) : 'No disponible'}`);
      ...
      const payload = {
        ...
        console.log('🔍 DEBUG API Brevo - Payload sender:', payload.sender);
        console.log('🔍 DEBUG API Brevo - Payload to:', payload.to);
        console.log('🔍 DEBUG API Brevo - API Key usada:', apiKey.substring(0, 10) + '...' + apiKey.substring(apiKey.length - 10));
      ...
      console.log('🔍 DEBUG API Brevo - Headers que se envían:');
      console.log('   accept:', headers.accept);
      console.log('   content-type:', headers['content-type']);
      console.log('   api-key:', headers['api-key'] ? 'Presente (longitud: ' + headers['api-key'].length + ')' : 'Ausente');
      
      console.log('🔍 DEBUG API Brevo - URL de destino:', 'https://api.brevo.com/v3/sendEmail');
      console.log('🔍 DEBUG API Brevo - Entorno:', process.env.NODE_ENV);
      console.log('🔍 DEBUG API Brevo - Node.js version:', process.version);
      console.log('🔍 DEBUG API Brevo - Platform:', process.platform);
      console.log('🔍 DEBUG API Brevo - User Agent:', process.env.npm_package_name + '/' + process.env.npm_package_version);

      // Crear agente HTTPS con opciones de seguridad más permisivas para Render
      ...
      console.error('   Request headers enviados:', error.config?.headers);
      console.error('   URL de la request:', error.config?.url);
      throw error;
    }
  }
