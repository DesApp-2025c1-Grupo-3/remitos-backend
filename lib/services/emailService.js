...
class EmailService {
  ...
  initialize() {
    ...
    if (process.env.EMAIL_HOST && process.env.EMAIL_USER && process.env.EMAIL_PASS) {
      ...
      this.transporter = nodemailer.createTransport({
        ...
        // Configuraciones TLS más permisivas para Render
        tls: {
        ...
        // Configuraciones de pool para Render
        pool: true,
        maxConnections: 1,
        maxMessages: 1,
  ...
  async sendMail(emailData) {
    try {
      ...
      console.log('🔍 DEBUG Gmail - Host:', process.env.EMAIL_HOST);
      console.log('🔍 DEBUG Gmail - Port:', process.env.EMAIL_PORT);
      console.log('🔍 DEBUG Gmail - User:', process.env.EMAIL_USER);
      console.log('🔍 DEBUG Gmail - Entorno:', process.env.NODE_ENV);
      console.log('🔍 DEBUG Gmail - Platform:', process.platform);
      console.log('🔍 DEBUG Gmail - Node version:', process.version);

      // Intentar con puerto 587 (TLS) primero
      try {
        console.log('🔍 DEBUG Gmail - Intentando con puerto 587 (TLS)...');
        await this.transporter.verify();
        console.log('✅ Conexión SMTP verificada exitosamente en puerto 587');
      } catch (tlsError) {
        console.log('⚠️ Puerto 587 falló, intentando con puerto 465 (SSL)...');
        console.log('   Error TLS:', tlsError.message);
        
        // Crear transporter alternativo con puerto 465 (SSL)
        ...
        console.log('✅ Conexión SMTP verificada exitosamente en puerto 465');
        this.transporter = sslTransporter;
      }
