...
// Endpoint específico para probar API de Brevo
app.get("/api/email/test-brevo-api", async (req, res) => {
  try {
    const emailService = require("./services/emailService");
    
    console.log('🧪 Probando API de Brevo...');
    
    // Crear datos de prueba
    const testEmailData = {
      to: 'gianbellone@gmail.com',
      subject: 'Test API Brevo desde Render',
      html: '<p>Este es un email de prueba desde la API de Brevo en Render</p>',
      text: 'Este es un email de prueba desde la API de Brevo en Render'
    };
    
    // Probar API directamente
    const result = await emailService.sendMailViaAPI(testEmailData);
    
    res.status(200).json({
      success: true,
      message: 'API de Brevo funcionando correctamente',
      result: result,
      timestamp: new Date().toISOString()
    });
    
  } catch (error) {
    console.error('❌ Error probando API de Brevo:', error);
    res.status(500).json({
      success: false,
      message: `Error probando API de Brevo: ${error.message}`,
      error: {
