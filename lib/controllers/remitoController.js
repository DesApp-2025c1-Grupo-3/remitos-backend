...
const createRemitoWithClienteAndDestino = async (req, res) => {
  ...
  try {
    ...
    // Crear Remito
    const estadoId = 8; // Estado por defecto
...
const updateRemito = async (req, res) => {
  ...
  try {
    ...
    // Validar agenda solo si el estado es "Autorizado"
    if (data.fechaAgenda && remitoUpdateData.estadoId) {
      ...
      if (estado && estado.nombre === "Autorizado") {
        ...
        console.log("✅ Se guardó fechaAgenda porque el estado es Autorizado");
      } else {
    ...
    // ---- MERCADERÍAS ----
    if (Array.isArray(data.mercaderias)) {
      console.log("🔍 DEBUG - Mercaderías recibidas:", data.mercaderias);
      ...
      const idsEnviados = data.mercaderias.filter(m => m.id).map((m) => m.id);

      // 1. Actualizar o crear mercaderías
      for (const m of data.mercaderias) {
