...
const createRemitoWithClienteAndDestino = async (req, res) => {
  ...
  try {
    ...
    // VALIDAR tipoMercaderiaId
    if (!tipoMercaderiaId) {
      return res.status(400).json({ 
        message: 'tipoMercaderiaId es requerido' 
      });
    }

    // Verificar que el tipo de mercadería existe
    const { TipoMercaderia } = require("../models");
    const tipoMercaderiaExiste = await TipoMercaderia.findByPk(tipoMercaderiaId);
    if (!tipoMercaderiaExiste) {
      return res.status(400).json({ 
        message: `Tipo de mercadería con ID ${tipoMercaderiaId} no encontrado` 
      });
    }

    // DEBUG: Log de datos extraídos
    console.log("🔍 DEBUG - Datos extraídos:");
    console.log("  - tipoMercaderiaId:", tipoMercaderiaId);
    ...
    const mercaderiaData = {
      tipoMercaderiaId: tipoMercaderiaId,  // Usar el ID directamente
    ...
    if (remito.mercaderiaId && Object.keys(mercaderiaUpdateData).length > 0) {
      ...
      if (mercaderia) {
        ...
        const mercaderiaUpdateDataProcessed = {
          ...
          tipoMercaderiaId: mercaderiaUpdateData.tipoMercaderiaId ? parseInt(mercaderiaUpdateData.tipoMercaderiaId) : null,
