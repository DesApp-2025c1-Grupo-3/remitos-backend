...
const mercaderiaSchema = Joi.object().keys({
  ...
  cantidadBobinas: Joi.number().integer().optional().allow(null).min(0).messages({
  ...
  cantidadBultos: Joi.number().integer().optional().allow(null).min(0).messages({
  ...
  cantidadPallets: Joi.number().integer().optional().allow(null).min(0).messages({
  ...
  requisitosEspeciales: Joi.string().optional().allow(null, "").max(500).messages({
    "string.base": "Requisitos especiales debe ser un texto",
    "string.max": "Requisitos especiales no puede tener más de 500 caracteres"
  })
});
module.exports = mercaderiaSchema;
//# sourceMappingURL=mercaderiaSchema.js.map