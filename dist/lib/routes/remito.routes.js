...
const upload = require("../middlewares/upload"); // Middleware para manejar errores de multer
...
route.post(
  "/remitoFinal",
  upload.single("archivoAdjunto"),
  handleMulterError, //schemaValidator(remitoSchema),
  remitoMiddleware.validateNumeroAsignadoUnico,
...
route.get("/archivo/:filename", (req, res) => {
  const { filename } = req.params; // Validar que el nombre del archivo sea seguro
  ...
  const filePath = path.join(__dirname, "../../uploads/remitos", filename); // Verificar que el archivo existe
